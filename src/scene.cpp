...
void Scene::DrawText(const std::string& text, const Vector2& world, int fontSize, const Color& color) const
{
	...
	::DrawText(text.c_str(), 0, 0, fontSize, color); //:: in front of DrawText is a scope opperator. It tells it to use the gloabl 
													 //funciton instead of the one that it is in(which would cause recursion)
	rlPopMatrix();
}
